<script>
 ...
 $: sortable=!presetFilter&&(definitions[table]||[]).some(([k])=>k==='sort')&&page===0&&count<=25&&Object.values(filters).every(v=>!v);
 let dragIndex=null;
 function dragStart(i){dragIndex=i;}
 async function dropOn(i){if(dragIndex===null||dragIndex===i)return;const reordered=[...rows];const [moved]=reordered.splice(dragIndex,1);reordered.splice(i,0,moved);dragIndex=null;rows=reordered;busy=true;try{await Promise.all(reordered.map((r,idx)=>supabase.from(table).update({sort:idx+1}).eq('id',r.id)));}catch(e){error=e.message;}busy=false;load(table,page,filters);}
 $: reset(table);
 ...
 async function load(t,p,f){const seq=++version;busy=true;error='';const sortCol=t==='Jobs'?'seq':t==='Statuses'?'sort':definitions[t][0][0];const sortOpts=t==='Jobs'?{ascending:false}:t==='Statuses'?{ascending:true,nullsFirst:false}:{ascending:true};let q=supabase.from(t).select(t==='Jobs'?'*,Companies(nameCompany),Statuses(status)':t==='People'?'*,Companies(nameCompany)':'*',{count:'exact'}).order(quoteIdentifier(sortCol),sortOpts).range(p*25,p*25+24);
...
<div class="panel table-wrap"><table><thead><tr>{#if sortable}<th></th>{/if}{#each columns as [key,type,label]}<th>{label||key}
 ...
 {#if busy}<tr><td colspan="8">Loading…</td></tr>{:else}{#each rows as row, i}<tr class="row-link" tabindex="0" role="button" aria-label={`Open ${rowLabel(row)}`} on:click={()=>open(row.id)} on:keydown={e=>{if(e.key==='Enter')open(row.id);}} on:dragover={e=>{if(sortable)e.preventDefault();}} on:drop={()=>{if(sortable)dropOn(i);}}>{#if sortable}<td class="drag-handle" draggable="true" on:dragstart={()=>dragStart(i)} on:click|stopPropagation aria-label="Drag to reorder">⠿</td>{/if}{#each columns as [key,type]}<td>{type==='boolean'?(row[key]?'Yes':'No'):type==='account'?(row[key]?'Linked':'No account'):row[key]||'—'}</td>{/each}{#if table==='Jobs'}{#if !(presetFilter&&presetFilter.key==='id_Status')}<td>{row.Statuses?.status||'—'}</td>{/if}<td>{row.Companies?.nameCompany||'—'}</td>{/if}{#if table==='People'&&heading!=='Seekers'}<td>{row.Companies?.nameCompany||'—'}</td><td>{row.position||'—'}</td>{/if}</tr>{:else}<tr><td colspan="8">No records found.</td></tr>{/each}{/if}
 </tbody></table></div>
