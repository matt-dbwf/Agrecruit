<script>
 ...
 onMount(load);
 async function load(){busy=true;const r=await supabase.from('JobSeekers').select('id,People(id,nameFirst,nameLast)',{count:'exact'}).eq('id_Job',job).order('id').range(page*25,page*25+24);rows=r.data||[];count=r.count||0;error=r.error?.message||'';busy=false;}
...
{#if busy}<p>Loading…</p>{:else}<table><tbody>{#each rows as row}<tr><td>{row.People?.nameFirst} {row.People?.nameLast}</td><td class="right">{#if editing}<button class="text" on:click={()=>remove(row.id)}>Unlink</button>{/if}</td></tr>{:else}<tr><td>No Seekers linked.</td></tr>{/each}</tbody></table>{/if}
