<script>
 ...
 function name(row){return table==='Companies'?row.nameCompany:[row.nameFirst,row.nameLast].filter(Boolean).join(' ');}
 function schedule(){clearTimeout(timer);timer=setTimeout(search,300);}
 async function search(){const seq=++version;busy=true;error='';let q=supabase.from(table).select('*').eq(flag,true).limit(15);const t=searchText(term);if(t){q=table==='Companies'?q.ilike('nameCompany',`%${t}%`):q.or(`nameFirst.ilike.%${t}%,nameLast.ilike.%${t}%`);}const {data,error:e}=await q.order(quoteIdentifier(table==='Companies'?'nameCompany':'nameLast'));if(seq!==version)return;results=data||[];error=e?.message||'';busy=false;}
</script>
<div class="selector">
