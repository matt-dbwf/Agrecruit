<script>
 ...
 onMount(async()=>{if(id==='new'){form=table==='Jobs'?{Status:'Pending',id_Company:''}:table==='Companies'?{flag_Client:false}:table==='People'?{flag_Seeker:false}:table==='Employees'?{flag_Manager:false}:{};return;}const r=await supabase.from(table).select('*').eq('id',id).single();error=r.error?.message||'';original=r.data;form={...original};loading=false;});
...
{#each fields[table] as [key,label]}<label>{label}{#if editing&&key!=='email'} <span class="required">*</span>{/if}<input type={key==='email'?'email':'text'} bind:value={form[key]} readonly={!editing||busy} required={key!=='email'||!!form.id_User}/></label>{/each}
...
{#if table==='People'}<label class="check"><input type="checkbox" bind:checked={form.flag_Seeker} disabled={!editing||busy}/>Seeker</label>{/if}
