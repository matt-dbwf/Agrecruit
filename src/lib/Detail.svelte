<script>
 ...
 const fieldType=key=>key==='email'?'email':(key==='linkedInURL'||key==='website')?'url':(key==='dateOfBirth'||key==='dateStart')?'date':(key==='currentSalary'||key==='splitAgrecruit'||key==='salaryMin'||key==='salaryMax')?'number':'text';
 ...
 onMount(async()=>{if(id==='new'){form=table==='Jobs'?{status:'Pending',id_Company:'',id_Contact:'',id_Address:'',id_Employee:''}:table==='Companies'?{flag_Client:false}:table==='People'?{flag_Seeker:seekerMode}:table==='Employees'?{flag_Manager:false}:{};return;}const r=await supabase.from(table).select('*').eq('id',id).single();error=r.error?.message||'';original=r.data;form={...original};loading=false;});
 ...
 async function save(){busy=true;error='';try{let data;if(table==='Employees'){data=await employeeAction('save',{...(id==='new'?{}:{id}),...form});}else{const values={};for(const [key]of currentFields){if(key.startsWith('_section_'))continue;const t=fieldType(key);const raw=form[key];values[key]=(t==='number'||t==='date')?(raw===''||raw==null?null:raw):String(raw||'').trim();}if(table==='Jobs'){values.id_Company=form.id_Company;values.id_Contact=form.id_Contact||null;values.id_Address=form.id_Address||null;values.id_Employee=form.id_Employee||null;values.id_Industry=form.id_Industry||null;values.status='Pending';}if(table==='Companies')values.flag_Client=form.flag_Client===true;if(table==='People'){values.flag_Seeker=form.flag_Seeker===true;if(!seekerMode){values.id_Company=form.id_Company||null;values.position=String(form.position||'').trim()||null;}}const r=id==='new'?await supabase.from(table).insert(values).select().single():await supabase.from(table).update(values).eq('id',id).select().single();if(r.error)throw r.error;data=r.data;}original=data;form={...data};editing=false;if(id==='new')saved(data.id);}catch(e){error=e.message;}busy=false;}
...
<div class="form-grid">
{#each currentFields as [key,label,required]}{#if key.startsWith('_section_')}<h3 class="wide section-heading">{label}</h3>{:else if key==='terms'||key==='descriptionFull'||key==='instructions'||key==='experience'}<label class="wide">{label}<textarea bind:value={form[key]} readonly={!editing||busy} rows="4"></textarea></label>{:else if key==='id_Industry'}<div class="field">{label}<Selector table="Industries" nameField="nameIndustry" label="Industries" bind:value={form.id_Industry} disabled={!editing||busy}/></div>{:else}<label class={key==='descriptionBrief'?'wide':''}>{label}{#if editing&&isRequired(table,key,required,form)} <span class="required">*</span>{/if}<input type={(!editing&&fieldType(key)==='date')?'text':fieldType(key)} bind:value={form[key]} readonly={key==='status'||!editing||busy} required={isRequired(table,key,required,form)} on:click={e=>openPicker(key,e)}/></label>{/if}{/each}
...
<div class="wide field">Client {#if editing}<span class="required">*</span>{/if}<Selector table="Companies" flag="flag_Client" label="Clients" bind:value={form.id_Company} disabled={!editing||busy}/>{#if editing&&!form.id_Company}<small>Select a Company marked as a Client.</small>{/if}</div>
