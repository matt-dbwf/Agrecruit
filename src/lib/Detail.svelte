<script>
 ...
 onMount(async()=>{if(id==='new'){form=table==='Jobs'?{id_Company:'',id_Contact:'',id_Address:'',id_Employee:'',id_Industry:'',id_Status:''}:table==='Companies'?{flag_Client:false}:table==='People'?{flag_Seeker:seekerMode}:table==='Employees'?{flag_Manager:false}:table==='Statuses'?{sort:await nextSort()}:{};return;}const r=await supabase.from(table).select('*').eq('id',id).single();error=r.error?.message||'';original=r.data;form={...original};loading=false;});
