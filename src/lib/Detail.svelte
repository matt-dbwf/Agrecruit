<script>
 ...
 export let table;export let id;export let back;export let saved;export let canEditEmployees=true;export let seekerMode=false;
...
<div class="toolbar"><div><button class="text" on:click={back}>← {backLabel}</button><h1>{id==='new'?'New ':''}{recordLabel}{table==='Jobs'&&original?` #${original['seq']}`:''}</h1></div>{#if !editing&&!loading&&original&&(table!=='Employees'||canEditEmployees)}<button class="secondary" on:click={()=>{editing=true;createUser=false;}}>Edit</button>{/if}</div>
